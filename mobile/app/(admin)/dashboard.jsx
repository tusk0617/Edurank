      ...
      >
        ...
        <View style={styles.header}>
          <View>
            ...
            <Text style={styles.name}>{user?.nama || 'Admin'}</Text>
        ...
        {stats && (
          <View style={styles.statsRow}>
            <View style={[styles.statCard, { borderLeftColor: Colors.primary }]}>
              <Text style={styles.statNum}>{stats.total_siswa}</Text>
              <Text style={styles.statLabel}>Siswa</Text>
            </View>
            <View style={[styles.statCard, { borderLeftColor: Colors.secondary }]}>
              <Text style={styles.statNum}>{stats.total_guru}</Text>
        ...
        <TouchableOpacity style={styles.menuCard} onPress={() => router.push('/(admin)/akun')} activeOpacity={0.85}>
          <View style={[styles.menuIcon, { backgroundColor: '#FFF7ED' }]}>
            <Ionicons name="people" size={28} color={Colors.warning} />
          </View>
          <View style={styles.menuInfo}>
            <Text style={styles.menuTitle}>Kelola Akun</Text>
            <Text style={styles.menuDesc}>Tambah, hapus, dan reset password akun siswa & guru</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    backgroundColor: Colors.card, borderRadius: 16, padding: 20, marginBottom: 16, elevation: 3,
  },
