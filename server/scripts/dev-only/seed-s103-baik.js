/**
 * Buat user S103 dan seed dummy hasil assessment kategori "baik" (~85%).
 * Jalankan: node server/scripts/dev-only/seed-s103-baik.js
 */
...
// S103: 6-7/8 benar per ujian → rata-rata ~85% → kategori "baik"
const attempts = [
  {
    assessment_id: 1, percobaan_ke: 1, days_ago: 20,
    // 7/8 benar (salah soal 6)
    jawaban: [
      ...
      { soal_id: 6, dipilih: 'd' }, // SALAH (c)
      { soal_id: 7, dipilih: 'c' }, // BENAR
      { soal_id: 8, dipilih: 'a' }, // BENAR
    ],
  },
  {
    assessment_id: 2, percobaan_ke: 1, days_ago: 17,
    // 7/8 benar (salah soal 11)
    jawaban: [
      { soal_id: 9,  dipilih: 'b' }, // BENAR
      { soal_id: 10, dipilih: 'a' }, // BENAR
      { soal_id: 11, dipilih: 'a' }, // SALAH (b)
  ...
  {
    assessment_id: 3, percobaan_ke: 1, days_ago: 14,
    // 7/8 benar (salah soal 19)
    jawaban: [
      { soal_id: 17, dipilih: 'b' }, // BENAR
      { soal_id: 18, dipilih: 'c' }, // BENAR
      { soal_id: 19, dipilih: 'a' }, // SALAH (d)
      { soal_id: 20, dipilih: 'b' }, // BENAR
      { soal_id: 21, dipilih: 'a' }, // BENAR
      { soal_id: 22, dipilih: 'b' }, // BENAR
      { soal_id: 23, dipilih: 'c' }, // BENAR
      { soal_id: 24, dipilih: 'c' }, // BENAR
    ],
  },
  {
    assessment_id: 4, percobaan_ke: 1, days_ago: 10,
    // 6/8 benar (salah soal 41, 45)
    jawaban: [
      { soal_id: 41, dipilih: 'c' }, // SALAH (b)
  ...
  },
  {
    ...
    // 7/8 benar (salah soal 26)
    jawaban: [
      { soal_id: 25, dipilih: 'a' }, // BENAR
      { soal_id: 26, dipilih: 'b' }, // SALAH (a)
      { soal_id: 27, dipilih: 'b' }, // BENAR
      { soal_id: 28, dipilih: 'c' }, // BENAR
      { soal_id: 29, dipilih: 'a' }, // BENAR
      { soal_id: 30, dipilih: 'c' }, // BENAR
...
async function run() {
  // Buat user S103 jika belum ada
  let [[s103]] = await pool.query("SELECT id FROM users WHERE username='S103'");
  if (!s103) {
    const hash = await bcrypt.hash('password123', 10);
    const [res] = await pool.query(
      "INSERT INTO users (username, nama, password, role) VALUES ('S103','Siswa S103',?,'siswa')",
      [hash]
    );
    s103 = { id: res.insertId };
    console.log(`User S103 dibuat, id=${s103.id}`);
  } else {
    console.log(`User S103 sudah ada, id=${s103.id}`);
  }

  const userId = s103.id;
  ...
  for (const attempt of attempts) {
    ...
    const waktu_selesai = new Date(waktu_mulai.getTime() + 20 * 60 * 1000);
  ...
  const pct = (totalBenar / totalSoal * 100).toFixed(1);
  console.log(`\nS103 capaian: ${totalBenar}/${totalSoal} = ${pct}% → kategori: ${pct >= 80 ? 'baik ✓' : pct >= 65 ? 'cukup' : 'perlu perhatian'}`);
  process.exit(0);
}
